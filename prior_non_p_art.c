///Priority non pre-emptive with arrival time
...
void waiting_time_calculation(int n){
    ...
    while(pc < n){
        ...
        if(p[process].remaining_time == 0){
            ...
            selected = 0;
            prior = INT_MAX;
            p[process].waiting_time = time - p[process].burst_time - p[process].arrival_time;
...
int main(){
    ...
    printf("PID\tBURST-TIME\tPRIORITY\tARRIVAL-TIME\tWAITING-TIME\tTURN-AROUND-TIME\n");
    for(i=0;i<n;i++){
        printf("%3d\t%10d\t%8d\t%12d\t%12d\t%16d\t\n",p[i].pid,p[i].burst_time,p[i].priority,p[i].arrival_time,p[i].waiting_time,p[i].turnaround_time);
    }
