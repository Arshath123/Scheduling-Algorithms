///Priority non pre-emptive without arrival time
...
struct process{
    int pid, burst_time, priority, waiting_time, turnaround_time,remaining_time;
}*p,temp;
...
int main(){
    ...
    for(i=0;i<n;i++){
        printf("\nEnter the process id,burst time,priority of %d th process\n",i+1);
        scanf("%d%d%d",&p[i].pid,&p[i].burst_time,&p[i].priority);
    }

    sort_with_priority(n);
